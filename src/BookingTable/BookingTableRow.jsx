...
import { singledeleted } from "../redux/actions";

const BookingTableRow = ({ booking }) => {
  const { from, to, date, guest, category, id } = booking;
  const distpatch = useDispatch();
  const handleDelete = (id) => {
    distpatch(singledeleted(id));
  };
    ...
    <>
      <tbody className="divide-y divide-gray-300/20" id="lws-previewBooked">
        <tr className="lws-bookedTable text-black">
          ...
          <td className="px-6 py-4 text-center">
            <span className="lws-bookedClass"> {category} </span>
          </td>
          <td className="px-6 py-4 text-center">
